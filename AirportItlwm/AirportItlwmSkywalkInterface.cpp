...
getTXPOWER(struct apple80211_txpower_data *txd)
{
    ...
    // v9: instrument-only (no extra zeroing past sizeof(*txd) — v8 crashed
    // kernel because internal callers can pass small txpower-only buffers
    // and writing 32 bytes overruns them). Counter bumps on every call so
    // we can confirm whether this method is on airportd's SUPPORTED_CHANNELS
    ...
    txd->txpower = 0;  // KEEP ZERO — see v7 comment above
