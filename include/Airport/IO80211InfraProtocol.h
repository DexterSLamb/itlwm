...
public:
    ...
    // Phase 3.7 Step 3: full Sequoia 15 InfraProtocol PV alignment.
    // 184 of 190 slots ground-truthed from KDK 15.7.4 apple80211 helper
    // disassembly (verified equivalent to BootKC 15.7.5 byte-by-byte).
    // 8 slots whose intended method couldn't be resolved are placeholders:
    // _reservedSlot_0xNNN. Source data:
    //   research/sequoia-port/ghidra-projects/infra-protocol-pv-slot-map.md
    //
    // Note: slots 0xe88 (= getCHANNEL via Apple's helper dispatch) and 0xe90
    // (= getPOWERSAVE via helper) live in IO80211InfraInterface concrete-method
    // region (= getWCL_TX_RX_LATENCY and createLQMData). The subclass overrides
    ...
    // sit at slots 0x1488+ — beyond Apple's vtable end (0x1480) — so don't
    // conflict with Apple's dispatch. Kept here to preserve subclass
    // (AirportItlwmSkywalkInterface) override declarations without churn.
    // For Sequoia 15, none of these are reachable via apple80211 helper
    // dispatch; they remain accessible through legacy code paths.
